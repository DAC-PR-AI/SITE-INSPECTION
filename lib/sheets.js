...
// Headers for the two project-config sheets
const PROJECTS_HEADERS = ["ProjectID", "ProjectName"];
const UNITS_HEADERS    = ["ProjectID", "UnitID"];
...
/**
 * getProjects() — joins the two admin-managed config sheets:
 *
 *   Projects sheet (master):  | ProjectID | ProjectName |
 *   Units sheet (detail):     | ProjectID | UnitID      |
 *
 * Returns the same map the rest of the app expects:
 *   { "ProjectName": ["Unit1", "Unit2", ...], ... }
 *
 * Admin workflow:
 *   1. Add a row in Projects to register a new project (give it a unique ProjectID).
 *   2. Add rows in Units, using the same ProjectID, to list its units.
 *   The app auto-refreshes every 60 s (PROJECTS_CACHE_TTL).
 */
export async function getProjects() {
  ...
  // ── 1. Ensure both sheets exist with correct headers ──────────────────────
  ...
  // ── 2. Load Projects master: ProjectID → ProjectName ─────────────────────
  const projectsRes = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID(),
    range: `${PROJECTS_SHEET}!A2:B200000`,
  });
  const projectIdToName = {};
  (projectsRes.data.values || []).forEach(([id, name]) => {
    const pid  = String(id   || "").trim();
    const pnam = String(name || "").trim();
    if (pid && pnam) projectIdToName[pid] = pnam;
  });

  // ── 3. Load Units detail: ProjectID → [UnitID, ...] ──────────────────────
  const unitsRes = await sheets.spreadsheets.values.get({
    spreadsheetId: SPREADSHEET_ID(),
    range: `${UNITS_SHEET}!A2:B200000`,
  });

  // ── 4. Join: build { ProjectName: ["Unit1", "Unit2", ...] } ───────────────
  const map = {};
  (unitsRes.data.values || []).forEach(([projectId, unitId]) => {
    const pid = String(projectId || "").trim();
    const uid = String(unitId   || "").trim();
    if (!pid || !uid) return;

    // Resolve project name from the master sheet; fall back to raw ID
    const projectName = projectIdToName[pid] || pid;
    if (!map[projectName]) map[projectName] = [];
    if (!map[projectName].includes(uid)) map[projectName].push(uid);
  });
