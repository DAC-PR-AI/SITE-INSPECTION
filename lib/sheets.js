...
async function findRow(sheets, inspectionId) {
  ...
  const target = String(inspectionId || "").trim().toLowerCase();
  const idx = rows.findIndex((r) => String(r?.[0] || "").trim().toLowerCase() === target);
...
async function findSignatureRow(sheets, inspectionId, role) {
  ...
  const rows = res.data.values || [];
  const target = String(inspectionId || "").trim().toLowerCase();
  const idx = rows.findIndex((r) => String(r?.[0] || "").trim().toLowerCase() === target && String(r?.[1] || "").trim().toLowerCase() === String(role || "").trim().toLowerCase());
  return idx === -1 ? null : idx + 2;
}

async function upsertSignatureRow(sheets, inspectionId, role, signerName, base64Data, timestamp = null) {
...
async function saveInspectionPhotos(sheets, data) {
  try {
    ...
    const proj = data.projectName || data.project || "";
    const unit = data.unitNumber || data.unit || "";
...
export async function getVerificationPhotoForInspection(inspectionId) {
  ...
  try {
    ...
    const res = await sheets.spreadsheets.values.get({
      spreadsheetId: photoSpreadsheetId,
      range: `${INSPECTION_PHOTOS_SHEET}!A2:G200000`,
    });
    const rows = res.data.values || [];
    ...
    return photoRow ? photoRow[6] : null;
  } catch (err) {
    console.warn("[sheets] Note on getVerificationPhotoForInspection:", err?.message);
    return null;
  }
}
...
export async function appendAuditRecord(auditRecord) {
  try {
    ...
    let project = auditRecord.project || auditRecord.projectName || "";
    let unit = auditRecord.unit || auditRecord.unitNumber || "";
    let inspectionType = auditRecord.inspectionType || "";

    if ((!project || !unit || !inspectionType) && auditRecord.inspectionId) {
      try {
        const insp = await getInspection(auditRecord.inspectionId);
        if (insp) {
          project = project || insp.projectName || insp.project || "";
          unit = unit || insp.unitNumber || insp.unit || "";
          inspectionType = inspectionType || insp.inspectionType || "INTERIOR JOINT INSPECTION";
        }
      } catch {}
    }
...
export async function upsertInspection(data, { submitting = false } = {}) {
  ...
  for (const [role, value] of getPersistableSignatures(signaturesRaw)) {
    ...
    if (base64String) {
      const sigTs = (typeof value === "object" && value !== null && value.timestamp) ? value.timestamp : null;
      signaturePromises.push(
        upsertSignatureRow(sheets, data.inspectionId, role, specificSigner, base64String, sigTs)
      );
  ...
  const now = new Date().toISOString();
  const inspectionDate = data.inspectionDate || data.date || now.slice(0, 10);
  const inspectionTime = data.inspectionTime || data.time || new Date().toTimeString().slice(0, 5);
  const workflowStatus = data.workflowStatus || (submitting ? "SITE_ENGINEER_PENDING" : (data.status || "draft"));

  // Check if existing record has submittedAt to NEVER lose or overwrite it on subsequent approvals
  const existingRow = await findRow(sheets, data.inspectionId);
  let existingSubmittedAt = data.submittedAt || "";
  if (existingRow && !existingSubmittedAt) {
    try {
      const existingRes = await sheets.spreadsheets.values.get({
        spreadsheetId: SPREADSHEET_ID(),
        range: `${INSPECTIONS_SHEET}!O${existingRow}:O${existingRow}`,
      });
      const val = existingRes.data.values?.[0]?.[0];
      if (val && String(val).trim()) {
        existingSubmittedAt = String(val).trim();
      }
    } catch {}
  }

  const effectiveSubmittedAt = existingSubmittedAt || (submitting ? now : "");
...
export async function getInspection(inspectionId) {
  ...
  if (cache.inspections.data) {
    const cachedItem = cache.inspections.data.find(i => (i.inspectionId || "").trim().toLowerCase() === targetId.toLowerCase());
    if (cachedItem) {
  ...
  if (!inspectionObj.customerVerificationPhoto) {
    const photo = await getVerificationPhotoForInspection(targetId);
    if (photo) {
      inspectionObj.customerVerificationPhoto = photo;
    }
  }
