...
export class GoogleSheetAdapter {
  ...
  static sheetRowToInspection(row = []) {
    ...
    return {
      ...
      projectName: project || parsedData.projectName || parsedData.project || "Default Project",
      unitNumber: unit || parsedData.unitNumber || parsedData.unit || "000",
      ...
      date: date || parsedData.inspectionDate || parsedData.date || new Date().toISOString().split("T")[0],
      inspectionDate: date || parsedData.inspectionDate || parsedData.date || new Date().toISOString().split("T")[0],
      time: time || parsedData.inspectionTime || parsedData.time || "10:00 AM",
      inspectionTime: time || parsedData.inspectionTime || parsedData.time || "10:00 AM",
      workflowStatus: parsedData.workflowStatus || (status && status !== "submitted" && status !== "draft" ? status : null) || parsedData.status || status || "DRAFT",
      ...
      submittedAt: submittedAt || parsedData.submittedAt || parsedData.createdAt || "",
      ...
      customerVerificationPhoto: parsedData.customerVerificationPhoto || parsedData.verificationPhoto || parsedData.handoverPhoto || (typeof parsedData.photos === "object" ? parsedData.photos?.customerVerification : "") || "",
      interiorDays: parsedData.interiorDays !== undefined && parsedData.interiorDays !== null && String(parsedData.interiorDays).trim() !== ""
        ? String(parsedData.interiorDays).trim()
        : (parsedData.days !== undefined && parsedData.days !== null && String(parsedData.days).trim() !== ""
            ? String(parsedData.days).trim()
            : (parsedData.interiorWorkDays !== undefined && parsedData.interiorWorkDays !== null && String(parsedData.interiorWorkDays).trim() !== ""
                ? String(parsedData.interiorWorkDays).trim()
                : "")),
      generalRemarks: parsedData.generalRemarks || "",
    };
  }
  ...
  static inspectionToSheetRow(inspection = {}) {
    ...
    const project = inspection.projectName || inspection.project || "Default Project";
    const unit = inspection.unitNumber || inspection.unit || "000";
    const inspectionType = inspection.inspectionType || "INTERIOR JOINT INSPECTION";
    const customerName = inspection.customerName || "N/A";
    const date = inspection.inspectionDate || inspection.date || new Date().toISOString().split("T")[0];
    const time = inspection.inspectionTime || inspection.time || "10:00 AM";
    ...
    const submittedAt = inspection.submittedAt || inspection.createdAt || updatedAt;
    ...
    const jsonStr = JSON.stringify({
      ...
      status,
      updatedAt,
      customerVerificationPhoto: inspection.customerVerificationPhoto || inspection.verificationPhoto || inspection.handoverPhoto || (typeof inspection.photos === "object" ? inspection.photos?.customerVerification : "") || "",
      interiorDays: inspection.interiorDays !== undefined && inspection.interiorDays !== null && String(inspection.interiorDays).trim() !== ""
        ? String(inspection.interiorDays).trim()
        : (inspection.days !== undefined && inspection.days !== null ? String(inspection.days).trim() : ""),
    });
