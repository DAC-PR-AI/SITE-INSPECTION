...
export async function upsertInspection(data, { submitting = false } = {}) {
  ...
  const status = data.workflowStatus || (submitting
    ? "QA_QC_PENDING"
    : (data.status || existing.status || "draft"));

  // ─── SIGNATURE EXTRACTION ─────────────────────────────────────────────────
  const signaturesRaw = data.signatures || {};
  const signaturesMetadata = { ...(existing.signatures || {}) };
  ...
  for (const [role, value] of Object.entries(signaturesRaw)) {
    ...
    if (typeof value === "string") {
      if (value.startsWith("data:") || value.length > 50) base64String = value;
    } else if (typeof value === "object" && value !== null) {
      if (value.dataUrl && typeof value.dataUrl === "string" && (value.dataUrl.startsWith("data:") || value.dataUrl.length > 50)) {
        base64String = value.dataUrl;
      } else if (value.signature && typeof value.signature === "string" && (value.signature.startsWith("data:") || value.signature.length > 50)) {
        base64String = value.signature;
      } else if (value.sigData && typeof value.sigData === "string" && (value.sigData.startsWith("data:") || value.sigData.length > 50)) {
        base64String = value.sigData;
      }
    }

    if (base64String) {
      sigDb[data.inspectionId][role] = base64String;
      if (typeof value === "object" && value !== null) {
        signaturesMetadata[role] = { ...value, dataUrl: base64String, status: "signed" };
      } else {
        signaturesMetadata[role] = base64String;
      }
    } else if (value && !signaturesMetadata[role]) {
