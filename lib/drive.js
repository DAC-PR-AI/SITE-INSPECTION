...
async function getOrCreateSubfolder(drive, parentId, folderName) {
  ...
  try {
    ...
    const q = `'${parentId}' in parents and mimeType='application/vnd.google-apps.folder' and name='${cleanName}' and trashed=false`;
    ...
    console.warn(`[drive] Could not create subfolder ${folderName}:`, err.message);
    return parentId;
  }
}

export async function uploadPhotoToDrive({ filename, dataUrl, project, unit }) {
  try {
    ...
    // Determine target subfolder (Project / Unit)
    let targetFolderId = rootFolderId;
    if (project) {
      const projFolderId = await getOrCreateSubfolder(drive, rootFolderId, project);
      if (unit) {
        targetFolderId = await getOrCreateSubfolder(drive, projFolderId, unit);
      } else {
        targetFolderId = projFolderId;
      }
    }
